...
:Menu
cls
title Portable UPlay Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall uplay [will remove uplay entirely]
echo 2. launch uplay [launches uplay]
echo 3. reset uplay [will remove everything uplay except the binary]
echo 4. uninstall uplay [i dont have a comment.]
...
echo d. check for new uplay version [automatically check for a new version]
...
:2
:LaunchUPlay
if not exist ".\bin\uplay\UbisoftConnect.exe" set "nag=PLEASE INSTALL UPLAY FIRST" & exit /b 2
title DO NOT CLOSE
cls
...
cls
title Portable UPlay Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable UPlay Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable UPlay Launcher - Helper Edition - Download Suite
...
title Portable UPlay Launcher - Helper Edition - Quicklauncher Writer
...
call :HelperDownload "https://ubi.li/4vxt9" "4vxt9"
:MoveUPlay
...
:e
title Portable UPlay Launcher - Helper Edition - Text-Reader Update Check
cls
...
:f
cd ".\data\Users\MarioMasta64\AppData\Local\Ubisoft Game Launcher\"
call :HelperReplaceText "settings.yml" ": " "$ "
for /f "delims=*" %%A in (settings.yml) do (
  set "A=%%A"
  for /f "tokens=1 delims=$" %%B in ("%%A") do (
    ...
    set "AP1=!A:~2,22!"
    set "AP2=!A:~2,20!"
    if "!AP1:~-5!" equ "_path" set "pathtrue=1"
    if "!AP2:~-5!" equ "_path" set "pathtrue=1"
    REM if "!pathtrue!" equ "1" echo "!B!"
    if "!pathtrue!" equ "1" (
      set "C=!B!"
      if "!C:~0,1!" NEQ "C" (
        if "!C:~0,1!" NEQ "A" (
                  ...
                  if "%%J" neq "No such Root Directory" (
                    ...
                    if "!D:~0,1!" NEQ "!I:~0,1!" (
                      ...
                      if exist "!I!!K:~1!" (
                        ...
                        call :HelperReplaceText "settings.yml" "!D!" "!I:~0,1!"
                      )
                    )
                  )
                  )
...
if not exist ".\bin\uplay\UbisoftConnect.exe" set nag=UPLAY IS NOT INSTALLED CHOOSE "D"
...
echo ================================================== >> !license!
cls
title Portable UPlay Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable UPlay Launcher - Helper Edition - Latest Build :D
...
:PreviewBuild
cls
title Portable UPlay Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable UPlay Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
