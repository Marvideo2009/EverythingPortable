...
:Menu
cls
title Portable Lunascape Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall lunascape [will remove lunascape entirely]
echo 2. launch lunascape [launches lunascape]
echo 3. reset lunascape [will remove everything lunascape except the binary]
echo 4. uninstall lunascape [tired of headlines in your toolbars?]
...
echo d. check for new lunascape version [automatically check for a new version]
...
call :UpgradeLunascape
exit /b 2

:2
:LaunchLunascape
if not exist ".\bin\lunascape\Luna.exe" set "nag=PLEASE INSTALL LUNASCAPE FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo lunascape IS RUNNING
start .\bin\lunascape\Luna.exe
exit
...
cls
title Portable Lunascape Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable Lunascape Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable Lunascape Launcher - Helper Edition - Download Suite
...
title Portable Lunascape Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo start .\bin\lunascape\Luna.exe>>!quick_launcher!
...
call :HelperDownload "https://api.github.com/repos/lunascape/orion-installer/releases/latest" "latest"
:: create file or wont work (do not run on same file)
:: create file or wont work (do not run on same file)
echo.> latest.txt
TYPE latest | MORE /P > latest.txt
for /f tokens^=4delims^=^" %%A in (
  'findstr /i /c:"_ml11_gl_full.exe" latest.txt'
) Do > .\doc\lunascape_link.txt Echo:%%A
if exist latest del latest >nul
...
if exist "!lunascape_exe!" "!lunascape_exe!" >nul
if exist ".\extra\!lunascape_exe!" (
  echo lunascape is updated.
  pause
  exit /b
)
call :HelperDownload "!lunascape_link!" "!lunascape_exe!"
:MoveLunascape
move "!lunascape_exe!" ".\extra\!lunascape_exe!"
:ExtractLunascape
call :HelperExtract7Zip "!folder!\extra\!lunascape_exe!" "!folder!\bin\lunascape\"
...
REM tested with LunaSetup6152_ml11_gl_full.exe
call :HelperDownload "https://mariomasta64.me/misc/lunascape6_locales.7z"  "lunascape6_locales.7z"
:VerifyLunascapeLocales
set /a count=1 
for /f "skip=1 delims=:" %%a in ('CertUtil -hashfile "!folder!\lunascape6_locales.7z" SHA256') do (
  if !count! equ 1 set "hash=%%a"
  set /a count+=1
)
echo !hash!
set "hash=%hash: =%"
if "!hash!" equ "62cc270902ce49c53dd49964290059ea77b231f04abf68cf5e1365219ceffcf9" goto MoveLunascapeLocales
del lunascape6_locales.7z
cls
echo Lunascape Locales Hash Does Not Match (Press "5" To Check For Launcher Updates And Re-Run "d" If One Is Found)
pause & exit /b 2
:MoveLunascapeLocales
move lunascape6_locales.7z .\extra\lunascape6_locales.7z
:ExtractLunascapeLocales
call :HelperExtract7Zip "!folder!\extra\lunascape6_locales.7z" "!folder!\bin\lunascape\"
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\!lunascape_exe!" & echo.>".\extra\lunascape6_locales.7z")
exit /b 2

:e
title Portable Lunascape Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\lunascape\Luna.exe" set nag=LUNASCAPE IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable Lunascape Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable Lunascape Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable Lunascape Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable Lunascape Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable Lunascape Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable Lunascape Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
