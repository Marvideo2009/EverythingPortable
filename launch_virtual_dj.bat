...
:2
:LaunchVirtualDJ
if not exist ".\bin\virtualdj\virtualdj.exe" set "nag=PLEASE INSTALL VIRTUAL DJ FIRST" & exit /b 2
title DO NOT CLOSE
cls
...
echo cls>>!quick_launcher!
echo if not exist .\data\virtualdj\ mkdir .\data\virtualdj\>>!quick_launcher!
echo cd .\data\virtualdj\>>!quick_launcher!
echo echo yes ^| reg delete HKEY_CURRENT_USER\SOFTWARE\VirtualDJ /v HomeFolder>>!quick_launcher!
echo reg add HKCU\SOFTWARE\VirtualDJ /v HomeFolder /d %%folder%%\data\virtualdj\>>!quick_launcher!
echo start "" "%%folder%%\bin\virtualdj\virtualdj.exe">>!quick_launcher!
...
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\VirtualDJ.msi")
exit /b 2
