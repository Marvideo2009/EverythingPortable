...
:Menu
cls
title Portable Discord Canary Launcher - Helper Edition - Main Menu
echo !NAG!
...
echo d. check for new discord version [automatically check for a new version]
...
:1
:ReinstallDiscordCanary
cls
call :UninstallDiscordCanary
call :UpgradeDiscordCanary
exit /b 2
...
:a
:DLLDownloaderCheck
cls & title Portable Discord Canary Launcher - Helper Edition - Download Dll Downloader
...
title Portable Discord Canary Launcher - Helper Edition - Quicklauncher Writer
...
for %%d in ("!folder!\temp\DiscordCanary*.nupkg") do set "discord=%%d"
echo !discord! & pause
...
:Credits
cls
if exist .\doc\discord_license.txt exit /b 2
echo ================================================== > .\doc\discord_license.txt
echo =              Script by MarioMasta64            = >> .\doc\discord_license.txt
set "extra_space="
if %current_version% LSS 10 set "extra_space= "
echo =           Script Version: v%current_version%- release        %extra_space%= >> .\doc\discord_license.txt
echo ================================================== >> .\doc\discord_license.txt
echo =You may Modify this WITH consent of the original= >> .\doc\discord_license.txt
echo = creator, as long as you include a copy of this = >> .\doc\discord_license.txt
echo =      as you include a copy of the License      = >> .\doc\discord_license.txt
echo ================================================== >> .\doc\discord_license.txt
echo =    You may also modify this script without     = >> .\doc\discord_license.txt
echo =         consent for PERSONAL USE ONLY          = >> .\doc\discord_license.txt
echo ================================================== >> .\doc\discord_license.txt
cls
title Portable Discord Canary Launcher - Helper Edition - About
for /f "DELIMS=" %%i in (.\doc\discord_license.txt) do (echo %%i)
pause
call :PingInstall
exit /b 2
...
:LatestBuild
cls
title Portable Discord Canary Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable Discord Canary Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:PreviewBuild
cls
title Portable Discord Canary Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable Discord Canary Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
