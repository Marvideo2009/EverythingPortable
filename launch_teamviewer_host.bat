...
:Menu
cls
title Portable TeamViewer Host Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall teamviewer host [will remove teamviewer host entirely]
echo 2. launch teamviewer host [launches teamviewer_host]
echo 3. reset teamviewer host [will remove everything teamviewer host except the binary]
echo 4. uninstall teamviewer host [do people keep thinking youre trying to scam them?]
...
echo d. check for new teamviewer host version [automatically check for a new version]
...
:1
:ReinstallTeamViewerHost
cls
call :UninstallTeamViewerHost
call :UpgradeTeamViewerHost
exit /b 2

:2
:LaunchTeamViewerHost
if not exist ".\bin\teamviewer_host\TeamViewer.exe" set "nag=PLEASE INSTALL TEAMVIEWER HOST FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo TEAMVIEWER HOST IS RUNNING
cd .\bin\teamviewer_host\
...
cls
title Portable TeamViewer Host Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable TeamViewer Host Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable TeamViewer Host Launcher - Helper Edition - Download Suite
...
title Portable TeamViewer Host Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo cd .\bin\teamviewer_host\>>!quick_launcher!
echo start TeamViewer.exe>>!quick_launcher!
...
:d
cls
:UpgradeTeamViewerHost
cls
title Portable TeamViewer Host Launcher - Helper Edition - TeamViewer Host Update Check
if exist TeamViewer_Host_Setup.exe del TeamViewer_Host_Setup.exe >nul
call :HelperDownload "https://download.teamviewer.com/download/TeamViewer_Host_Setup.exe" "TeamViewer_Host_Setup.exe"
:MoveTeamViewerHost
move TeamViewer_Host_Setup.exe .\extra\TeamViewer_Host_Setup.exe
:ExtractTeamViewerHost
call :HelperExtract7Zip "!folder!\extra\TeamViewer_Host_Setup.exe" "!folder!\temp\"
call :HelperExtract7Zip "!folder!\temp\tvfiles.7z" "!folder!\bin\teamviewer_host\"
if exist .\bin\teamviewer_host\Uninstall.exe del .\bin\teamviewer_host\Uninstall.exe >nul
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\TeamViewer_Host_Setup.exe")
exit /b 2

:e
title Portable TeamViewer Host Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\teamviewer_host\TeamViewer.exe" set nag=TEAMVIEWER HOST IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable TeamViewer Host Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable TeamViewer Host Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable TeamViewer Host Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable TeamViewer Host Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable TeamViewer Host Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable TeamViewer Host Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
