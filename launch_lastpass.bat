...
:Menu
cls
title Portable LastPass Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall lastpass [will remove lastpass entirely]
echo 2. launch lastpass [launches lastpass]
echo 3. reset lastpass [will remove everything lastpass except the binary]
echo 4. uninstall lastpass [upset by lastpass nerfing the usefulness of their password manager?]
...
echo d. check for new lastpass version [automatically check for a new version]
...
:ReinstallLastPass
cls
call :UninstallLastPass
call :UpgradeLastPass
exit /b 2

:2
:LaunchLastPass
if not exist ".\bin\lastpass\lastapp.exe" set "nag=PLEASE INSTALL LASTPASS FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo LASTPASS IS RUNNING
start .\bin\lastpass\lastapp.exe
exit
...
if exist .\extra\lastappinstall.exe del .\extra\lastpassappinstall.exe >nul
exit /b 2
...
cls
title Portable LastPass Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable LastPass Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable LastPass Launcher - Helper Edition - Download Suite
...
title Portable LastPass Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo start .\bin\lastpass\lastapp.exe>>!quick_launcher!
...
call :HelperDownload "https://download.cloud.lastpass.com/windows_installer/lastappinstall.exe" "lastappinstall.exe"
:MoveLastPass
move lastappinstall.exe .\extra\lastappinstall.exe
:ExtractLastPass
call :HelperExtract7Zip "!folder!\extra\lastappinstall.exe" "!folder!\bin\lastpass\"
if exist .\bin\lastpass\$PLUGINSDIR\ rmdir /s /q .\bin\lastpass\$PLUGINSDIR\
if exist .\bin\lastpass\lastappuninstall.exe del .\bin\lastpass\lastappuninstall.exe.nsis >nul
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\lastappinstall.exe")
exit /b 2

:e
title Portable LastPass Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\lastpass\lastapp.exe" set nag=LASTPASS IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable LastPass Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable LastPass Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable LastPass Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable LastPass Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable LastPass Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable LastPass Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
