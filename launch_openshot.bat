...
:Menu
cls
title Portable OpenShot Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall openshot [will remove openshot entirely]
echo 2. launch openshot [launches openshot]
echo 3. reset openshot [will remove everything openshot except the binary]
echo 4. uninstall openshot [none of your friends on openshot?]
...
echo d. check for new openshot version [automatically check for a new version]
...
:ReinstallOpenShot
cls
call :UninstallOpenShot
call :UpgradeOpenShot
exit /b 2

:2
:LaunchOpenShot
if not exist ".\bin\openshot\!arch!\openshot-qt.exe" set "nag=PLEASE INSTALL OPENSHOT FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo OPENSHOT IS RUNNING
start "" ".\bin\openshot\!arch!\openshot-qt.exe"
exit
...
if exist .\extra\OpenShot* del .\extra\OpenShot* >nul
exit /b 2
...
cls
title Portable OpenShot Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable OpenShot Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable OpenShot Launcher - Helper Edition - Download Suite
...
title Portable OpenShot Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo start "" ".\bin\openshot\%%arch%%\openshot-qt.exe">>!quick_launcher!
...
call :HelperDownload "https://www.openshot.org/download/" "index.html"
set counter=0
:UpgradeSearchLoop
set /a counter+=1
for /f tokens^=%counter%delims^=^" %%A in (
  'findstr /i /c:"-x86_64.exe" index.html'
) Do > .\doc\openshot_link.txt Echo:%%A
set /p openshot_link=<.\doc\openshot_link.txt
if "!Debug!" EQU "1" (
  echo !openshot_link!
  echo !counter!
)
if "!openshot_link:~0,13!"=="//github.com/" ( if "!Debug!" EQU "1" ( echo hit ) ) & goto ExitUpgradeSearchLoop
goto UpgradeSearchLoop
:ExitUpgradeSearchLoop
if exist index.html del index.html >nul
set "openshot_link64=https:!openshot_link:.exe.torrent=.exe!"
set "openshot_link32=!openshot_link64:x86_64=x86!"
if "!Debug!" EQU "1" (
  cls
...
if exist ".\extra\!openshot_exe64!" (
  if exist ".\extra\!openshot_exe32!" (
    echo openshot is updated.
    pause
    exit /b 2
  )
)
cls
echo upgrading to openshot v!openshot_exe64:~10,-11!
call :HelperDownload "!openshot_link32!" "!openshot_exe32!"
call :HelperDownload "!openshot_link64!" "!openshot_exe64!"
:MoveOpenShot
move "!openshot_exe32!" ".\extra\!openshot_exe32!"
move "!openshot_exe64!" ".\extra\!openshot_exe64!"
:ExtractOpenShot
call :HelperExtractInno "!folder!\extra\!openshot_exe32!" "!folder!\temp32\"
call :HelperExtractInno "!folder!\extra\!openshot_exe64!" "!folder!\temp64\"
:aa
xcopy .\temp32\{app}\* .\bin\openshot\32\ /e /i /y
xcopy .\temp64\{app}\* .\bin\openshot\64\ /e /i /y
if exist .\temp32\ rmdir /s /q .\temp32\
if exist .\temp64\ rmdir /s /q .\temp64\
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\!openshot_exe32!" & echo.>".\extra\!openshot_exe64!")
exit /b 2

:e
title Portable OpenShot Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\openshot\!arch!\openshot-qt.exe" set nag=OPENSHOT IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable OpenShot Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable OpenShot Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable OpenShot Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable OpenShot Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable OpenShot Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable OpenShot Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
