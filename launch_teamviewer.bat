...
:Menu
cls
title Portable TeamViewer Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall teamviewer [will remove teamviewer entirely]
echo 2. launch teamviewer [launches teamviewer]
echo 3. reset teamviewer [will remove everything teamviewer except the binary]
echo 4. uninstall teamviewer [do people keep thinking youre trying to scam them?]
...
echo d. check for new teamviewer version [automatically check for a new version]
...
:ReinstallTeamViewer
cls
call :UninstallTeamViewer
call :UpgradeTeamViewer
exit /b 2

:2
:LaunchTeamViewer
if not exist ".\bin\teamviewer\TeamViewer.exe" set "nag=PLEASE INSTALL TEAMVIEWER FIRST" & exit /b 2
title DO NOT CLOSE
cls
...
if exist .\bin\teamviewer\TeamViewer\ rmdir /s /q .\bin\teamviewer\TeamViewer\
exit /b 2
...
cls
taskkill /f /im TeamViewer.exe
echo y | if exist .\bin\teamviewer\*.* del .\bin\teamviewer\*.* >nul
exit /b 2
...
cls
title Portable TeamViewer Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable TeamViewer Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable TeamViewer Launcher - Helper Edition - Download Suite
...
title Portable TeamViewer Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo cd .\bin\teamviewer\>>!quick_launcher!
...
if exist TeamViewerPortable.zip del TeamViewerPortable.zip >nul
call :HelperDownload "https://download.teamviewer.com/download/TeamViewerPortable.zip" "TeamViewerPortable.zip"
:MoveTeamViewer
move TeamViewerPortable.zip .\extra\TeamViewerPortable.zip >nul
:ExtractTeamViewer
call :HelperExtract "!folder!\extra\TeamViewerPortable.zip" "!folder!\bin\teamviewer\"
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\TeamViewerPortable.zip")
exit /b 2

:e
title Portable TeamViewer Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\teamviewer\TeamViewer.exe" set nag=TEAMVIEWER IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable TeamViewer Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable TeamViewer Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable TeamViewer Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:PreviewBuild
cls
title Portable TeamViewer Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable TeamViewer Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
