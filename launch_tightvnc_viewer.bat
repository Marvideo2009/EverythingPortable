...
:Menu
cls
title Portable TightVNC Viewer Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall tightvnc viewer [will remove tightvnc viewer entirely]
echo 2. launch tightvnc viewer [launches tightvnc viewer]
echo 3. reset tightvnc viewer [will remove everything tightvnc viewer except the binary]
echo 4. uninstall tightvnc viewer [want a different ui for vnc connecting?]
...
:1
:ReinstallTightVNCViewer
cls
call :UninstallTightVNCViewer
call :UpgradeTightVNCViewer
exit /b 2

:2
:LaunchTightVNCViewer
if not exist ".\bin\tightvnc_viewer\tvnviewer.exe" set "nag=PLEASE INSTALL TIGHTVNC VIEWER FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo TIGHTVNC VIEWER IS RUNNING
start .\bin\tightvnc_viewer\tvnviewer.exe
exit
...
cls
title Portable TightVNC Viewer Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable TightVNC Viewer Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable TightVNC Viewer Launcher - Helper Edition - Download Suite
...
title Portable TightVNC Viewer Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo start .\bin\tightvnc_viewer\tvnviewer.exe>>!quick_launcher!
...
:d
cls
:UpgradeTightVNCViewer
title Portable TightVNC Viewer Launcher - Helper Edition - TightVNC Viewer Update Check
if exist latest del latest >nul
if exist latest.txt del latest.txt >nul
if exist download del download >nul
call :HelperDownload "https://www.tightvnc.com/download.php" "download.php"
set counter=0
:UpgradeSearchLoop
set /a counter+=1
REM for /f tokens^=%counter%delims^=^" %%A in (
REM   'findstr /i /c:"https://cdn-fastly.tightvnc_viewerproject.com/downloads/tightvnc_viewer-Studio" download'
for /f tokens^=%counter%delims^=^" %%A in (
  'findstr /i /c:".msi" download.php'
) Do > .\doc\tightvnc_viewer_link.txt Echo:%%A
set /p tightvnc_viewer_link=<.\doc\tightvnc_viewer_link.txt
if "!Debug!" EQU "1" (
  echo !tightvnc_viewer_link!
  echo !counter!
)
if "!tightvnc_viewer_link:~0,34!"=="https://www.tightvnc.com/download/" ( if "!Debug!" EQU "1" ( echo hit ) ) & goto ExitUpgradeSearchLoop
...
if exist latest.txt del latest.txt >nul
cls
if exist ".\extra\!tightvnc_viewer_msi!" (
  echo tightvnc_viewer is updated.
  pause
  exit /b 2
)
cls
echo upgrading to tightvnc_viewer v!tightvnc_viewer_msi:~9,-20!
call :HelperDownload "!tightvnc_viewer_link!" "!tightvnc_viewer_msi!"
:MoveTightVNCViewer
move "!tightvnc_viewer_msi!" ".\extra\!tightvnc_viewer_msi!"
:ExtractTightVNCViewer
call :HelperExtract7Zip ".\extra\!tightvnc_viewer_msi!" "!folder!\bin\tightvnc_viewer\"
move ".\bin\tightvnc_viewer\screenHooksLoader" ".\bin\tightvnc_viewer\hookldr.exe"
move ".\bin\tightvnc_viewer\LicenseFile" ".\bin\tightvnc_viewer\LICENSE.txt"
move ".\bin\tightvnc_viewer\screenHooksFile" ".\bin\tightvnc_viewer\screenhooks32.dll"
move ".\bin\tightvnc_viewer\screenHooksFile64" ".\bin\tightvnc_viewer\screenhooks64.dll"
move ".\bin\tightvnc_viewer\UrlFile" ".\bin\tightvnc_viewer\TightVNC Web Site.url"
move ".\bin\tightvnc_viewer\tvnserverEXE" ".\bin\tightvnc_viewer\tvnserver.exe"
move ".\bin\tightvnc_viewer\viewerEXE" ".\bin\tightvnc_viewer\tvnviewer.exe"
if exist ".\bin\tightvnc_viewer\sasDllFile" del ".\bin\tightvnc_viewer\sasDllFile" >nul
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\!tightvnc_viewer_msi!")
exit /b 2

:e
title Portable TightVNC Viewer Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\tightvnc_viewer\tvnviewer.exe" set nag=TIGHTVNC VIEWER IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable TightVNC Viewer Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable TightVNC Viewer Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable TightVNC Viewer Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable TightVNC Viewer Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable TightVNC Viewer Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable TightVNC Viewer Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
