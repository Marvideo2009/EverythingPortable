...
:Menu
cls
title Portable WeChat Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall wechat [will remove wechat entirely]
echo 2. launch wechat [launches wechat]
echo 3. reset wechat [will remove everything wechat except the binary]
echo 4. uninstall wechat [none of your friends on wechat?]
...
echo d. check for new wechat version [automatically check for a new version]
...
:2
:LaunchWeChat
if not exist ".\bin\wechat\WeChat.exe" set "nag=PLEASE INSTALL WECHAT FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo WECHAT IS RUNNING
start .\bin\wechat\WeChat.exe
exit
...
cls
title Portable WeChat Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable WeChat Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable WeChat Launcher - Helper Edition - Download Suite
...
title Portable WeChat Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo start .\bin\wechat\WeChat.exe>>!quick_launcher!
...
call :HelperDownload "https://dldir1.qq.com/weixin/Windows/WeChatSetup.exe" "WeChatSetup.exe"
:MoveWeChat
move WeChatSetup.exe .\extra\WeChatSetup.exe
:ExtractWeChat
call :HelperExtract7Zip "!folder!\extra\WeChatSetup.exe" "!folder!\bin\wechat\"
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\WeChatSetup.exe")
exit /b 2

:e
title Portable WeChat Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\wechat\WeChat.exe" set nag=WECHAT IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable WeChat Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable WeChat Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable WeChat Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable WeChat Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable WeChat Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable WeChat Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
